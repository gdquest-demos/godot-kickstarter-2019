...
func setup() -> void:
	...
	"""
	var cell_size_px : = size * _tile_map.cell_size
	get_tree().set_screen_stretch(SceneTree.STRETCH_MODE_2D, SceneTree.STRETCH_ASPECT_KEEP, cell_size_px)
	OS.set_window_size(2 * cell_size_px)
...
func generate_perimeter() -> void:
	...
	Randomly selects from the tiles marked as `Cell.OUTER` using the funciton `pick_cell`.
	"""
...
func generate_inner() -> void:
	...
	`rand_cell` function that takes the probability for `Cell.GROUND` tiles to have some more control
	over what types of tiles we'll be placing.
	"""
	for x in range(1, size.x - 1):
		for y in range(1, size.y - 1):
			var cell : = rand_cell(ground_probability)
...
func pick_cell(type: int) -> int:
	...
	elif type == Cell.GROUND:
		interval = Vector2(10, 14)
	elif type == Cell.OBSTACLE:
...
func rand_cell(probability: float) -> int:
	...
	Returns the id of the cell in the TileSet resource.
	"""
	return pick_cell(Cell.GROUND) if _rng.randf() < probability else pick_cell(Cell.OBSTACLE)
